        ...
        public static XElement ToXElement(this object data, string root)
        {
            ...
            else
            {
                ...
                var elements = from prop in props
                               ...
                               let value = prop.PropertyType.IsSimpleType() ? (name == "ReturnCode" ? (Enum.TryParse<ReturnCodes>(val.ToString(), out newVal) == true ? new XElement(name, (int)Enum.Parse(typeof(ReturnCodes), val.ToString())) : new XElement(name, -1)) : new XElement(name, val)) : val.ToXElement(name)
                               where value != null
                               select value;
